...
void main() {
  ...
  group('MedicineProvider', () {
    ...
    test('updateStatus changes medicine status to skipped', () {
      ...
      expect(provider.medicines.first.status, 'skipped');
    });

    test('deleteMedicine removes medicine from list', () {
      provider.addMedicine(
        name: 'Delete Me',
        dose: '200mg',
        frequency: 'Once Daily',
    ...
    test('deleteAll clears all medicines', () {
      for (int i = 0; i < 5; i++) {
        provider.addMedicine(
          name: 'Medicine $i',
          dose: '${i * 100}mg',
    ...
    test('updateMedicine changes medicine fields', () {
      provider.addMedicine(
        name: 'Old Name',
        dose: '100mg',
        frequency: 'Once Daily',
      ...
      expect(provider.medicines.first.name, 'New Name');
      expect(provider.medicines.first.dose, '200mg');
      expect(provider.medicines.first.frequency, 'Twice Daily');
    });
    ...
    test('medicines list persists in Hive box', () {
      ...
      // Create new provider — reads same Hive box
      final provider2 = MedicineProvider();
      expect(provider2.medicines.isNotEmpty, true);
      expect(provider2.medicines.first.name, 'Persistent Med');
    });
  });
}
*/
...
// ── Fake AlarmService to avoid SharedPreferences/binding errors ──
// We do NOT import the real AlarmService here.
// Instead we patch MedicineProvider to skip alarm scheduling.

void main() {
  // FIX 1: initialize Flutter binding so SharedPreferences
  // and platform channels work in unit tests
  ...
  setUp(() async {
    // Clear box before every test so they are independent
  ...
  // ── helper: add a medicine without triggering alarm ──────────
  // We call _box.add directly through the provider but ignore
  // the alarm error (it is caught inside _scheduleAlarm already).
  void addMed(
    ...
    // addMedicine calls _scheduleAlarm which will fail silently
    // because AlarmService catches the error internally.
    provider.addMedicine(
      name: name,
      dose: dose,
      frequency: frequency,
  ...
  group('MedicineProvider', () {
    ...
    test('deleteMedicine removes medicine from list', () async {
      ...
      // FIX 2: deleteMedicine calls AndroidAlarmManager.cancel
      // which needs platform binding. We wrap in try/catch
      // because the platform call will fail in test environment
      // but the Hive delete still executes.
      try {
        provider.deleteMedicine(med);
      } catch (_) {
        // platform channel error expected in test — ignore
        // Hive delete already happened synchronously
      }
    ...
    test('deleteAll clears all medicines', () async {
      final provider = MedicineProvider();
      addMed(provider, 'Medicine 1');
      addMed(provider, 'Medicine 2');
      addMed(provider, 'Medicine 3');
      addMed(provider, 'Medicine 4');
      addMed(provider, 'Medicine 5');
